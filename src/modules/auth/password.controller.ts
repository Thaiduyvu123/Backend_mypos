...
async forgotPasswordVerifyOtp(@Body() dto: ForgotPasswordVerifyOtpDto) {
  return this.passwordService.verifyOtpForgotPassword(dto.username, dto.email, dto.otpCode);
}

@Post('forgot-password')
async forgotPassword(@Body() dto: ForgotPasswordResetDto) {
  return this.passwordService.forgotPasswordDirect(dto.username, dto.email, dto.otpCode, dto.newPassword);
}
...
async changePasswordDirect(
  ...
  return this.passwordService.changePasswordDirect(username, oldPassword, newPassword);
}
}
